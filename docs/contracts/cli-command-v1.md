...
This table freezes routing identity and ownership only. T02 did not create concrete report bindings for `doctor`, `status`, `literature add/resume/review`, or `knowledge search/show`. [Operations v1](./operations-v1.md), [Literature Commands v1](./literature-commands-v1.md), and the T05 Knowledge Read contracts subsequently bound those seven paths; `knowledge.ask` remains independently bound by [Knowledge Ask Result v1](./knowledge-ask-result-v1.md) and [Knowledge Ask Diagnostics v1](./knowledge-ask-diagnostics-v1.md). All eight paths therefore have a concrete binding, while [T06 / Issue #7](https://github.com/Dulealex/Gezhi/issues/7) completes only the Ask binding's remaining supplemental/Human observable surface. This routing table neither replaces nor generalizes any concrete binding.
...
All eight public paths now have command-owned Human/JSON bindings; parser tests may therefore cross the route/raw-value seam only by invoking the selected owning contract, never by constructing a grammar-owned envelope. The existing `knowledge.ask` binding continues to use its authoritative result/diagnostic contracts; T06 owns only its remaining supplemental diagnostics, Human presentation/exit, Retrieval View overflow, and presentation edges. This statement does not claim that the complete public surface is production-ready.
