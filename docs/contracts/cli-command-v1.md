...
Context-only in this contract means business direct dependencies outside that bootstrap runtime import closure: Feedparser, HTTPX, Pydantic, Pydantic Settings, PyPDF, RapidFuzz, Tenacity, OCR, Codex, and future Context business dependencies. The routing descriptor and graph factory must not import or probe them, and only the selected handled adapter may lazy-load what its operation consumes.
