...
This contract owns public token spelling, arity, option placement, defaults, mutual exclusion, route-to-owning-module selection, launcher parity, and the boundary between entry failures and handled command outcomes. It does not itself bind a concrete `CliResultEnvelopeV1.command`, result object, diagnostic union, Human result copy, or domain validation. [Operations v1](./operations-v1.md) owns those concrete surfaces for `doctor` and `status`; [Literature Commands v1](./literature-commands-v1.md) owns all three Literature commands. Only `knowledge.search/show` remain for [T05 / Issue #6](https://github.com/Dulealex/Gezhi/issues/6). `knowledge.ask` keeps its existing [result](./knowledge-ask-result-v1.md) and [diagnostic](./knowledge-ask-diagnostics-v1.md) binding, while [T06 / Issue #7](https://github.com/Dulealex/Gezhi/issues/7) is limited to its remaining observable surface.
...
This table freezes routing identity and ownership only. T02 did not create or bind `CliResultEnvelopeV1.command` values for `doctor`, `status`, `literature add/resume/review`, or `knowledge search/show`. [Operations v1](./operations-v1.md) subsequently bound the first two and [Literature Commands v1](./literature-commands-v1.md) bound the three Literature paths. Only `knowledge.search/show` remain for [T05 / Issue #6](https://github.com/Dulealex/Gezhi/issues/6). `knowledge.ask` remains bound by [Knowledge Ask Result v1](./knowledge-ask-result-v1.md) and [Knowledge Ask Diagnostics v1](./knowledge-ask-diagnostics-v1.md); [T06 / Issue #7](https://github.com/Dulealex/Gezhi/issues/7) completes only that binding's remaining observable surface. This table neither replaces nor generalizes any concrete binding.
