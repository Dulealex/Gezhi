...
This contract owns public token spelling, arity, option placement, defaults, mutual exclusion, route-to-owning-module selection, launcher parity, and the boundary between entry failures and handled command outcomes. It does not itself bind concrete command reports. All eight public paths now have command-owned concrete bindings: [Operations v1](./operations-v1.md) owns `doctor` and `status`; [Literature Commands v1](./literature-commands-v1.md) owns all three Literature commands; [Knowledge Read v1](./knowledge-read-v1.md) and [Knowledge Read Diagnostics v1](./knowledge-read-diagnostics-v1.md) own `knowledge.search` and `knowledge.show`; and `knowledge.ask` keeps its existing [result](./knowledge-ask-result-v1.md) and [diagnostic](./knowledge-ask-diagnostics-v1.md) binding. [Knowledge Ask Observable v1](./knowledge-ask-observable-v1.md) now freezes the Ask supplemental, Human, Retrieval Audit/View measurement, and presentation-failure boundary without changing the existing result, primary, normal JSON path, or the total of eight public commands. This contract closure does not claim the implementation is production-ready.
...
This table freezes routing identity and ownership only. T02 did not create concrete report bindings for `doctor`, `status`, `literature add/resume/review`, or `knowledge search/show`. [Operations v1](./operations-v1.md), [Literature Commands v1](./literature-commands-v1.md), and the T05 Knowledge Read contracts subsequently bound those seven paths; `knowledge.ask` remains independently bound by [Knowledge Ask Result v1](./knowledge-ask-result-v1.md) and [Knowledge Ask Diagnostics v1](./knowledge-ask-diagnostics-v1.md). All eight paths therefore have a concrete binding; [Knowledge Ask Observable v1](./knowledge-ask-observable-v1.md) completes only the Ask binding's supplemental/Human/Audit/presentation observable surface and does not add a ninth route. This routing table neither replaces nor generalizes any concrete binding.
...
Mode is authoritative only after the complete leaf grammar succeeds. Configuration invalidity, Data Root failure, environment capability failure, and domain validation that occur after that point must respect the selected mode. The command-specific contracts own their concrete command identity, outcome, diagnostic, exit, and Human copy: [Operations v1](./operations-v1.md) closes those facts for `doctor/status`, [Literature Commands v1](./literature-commands-v1.md) closes them for `literature add/resume/review`, and [Knowledge Read v1](./knowledge-read-v1.md) plus [Knowledge Read Diagnostics v1](./knowledge-read-diagnostics-v1.md) close them for `knowledge search/show`. This grammar does not recreate any of those payloads.

All eight public paths now have command-owned Human/JSON bindings; parser tests may therefore cross the route/raw-value seam only by invoking the selected owning contract, never by constructing a grammar-owned envelope. The existing `knowledge.ask` binding continues to use its authoritative result/primary contracts; [Knowledge Ask Observable v1](./knowledge-ask-observable-v1.md) owns its supplemental diagnostics, Human presentation/exit, Retrieval Audit/View measurement, and presentation-failure boundary without changing that normal path. This statement does not claim that the complete public surface is production-ready.

Only `knowledge.ask --json` adopts the combined ADR 0107 through ADR 0109 candidate/seal/commit presentation package. [Operations v1](./operations-v1.md) separately and explicitly adopts the same numerical cap and ADR 0109 synchronous binary fd1 primitive only after its own read-only resources are settled; it does not inherit the Knowledge cancellation seal, Answer commit semantics, or manifest parity. [Literature Commands v1](./literature-commands-v1.md) owns a third, independent settled-resource presentation profile for its three bindings. [Knowledge Read v1](./knowledge-read-v1.md) owns a fourth profile for `knowledge search/show`: a 1,048,576-byte inclusive complete-envelope cap and its concrete binary writer, without Ask cancellation, Answer commit, or manifest-parity semantics.
...
| ADR 0089 / ADR 0091 | handled JSON outer and diagnostic item remain separate from entry failures; T02 itself did not bind seven concrete command identities, which later command-owned contracts now bind |
