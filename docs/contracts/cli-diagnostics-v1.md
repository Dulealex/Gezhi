...
状态：共享 item、数组角色、排序、容量、omission 与隐私 profile 已冻结；`knowledge.ask` committed Answer 的 15+1 primary subset、no-commit 的 outcome/result/正常 JSON exit，以及 no-commit `blocked` 的 11 项、`failed` 的 7 项与 `interrupted` 的 1 项 primary/context 已由 [Knowledge Ask Diagnostics v1](./knowledge-ask-diagnostics-v1.md) 冻结，blocked 内部 fail-fast 仲裁、跨 outcome 静态优先级、cancellation latch/checkpoints、atomic pre-ID barrier 与 `NoCommitSafeBoundaryV1` 也已冻结。[Operations v1](./operations-v1.md) 已冻结 `doctor/status` 的完整 V1 code/context union、角色、优先级、outcome/result/exit 与 Human 映射；[Literature Commands v1](./literature-commands-v1.md) 已完整冻结 `literature.add`、`literature.resume` 与 `literature.review` 的 primary union/context、result presence、无 supplemental 决定、静态仲裁、Human 文案与 normal exit；[Knowledge Read Diagnostics v1](./knowledge-read-diagnostics-v1.md) 已冻结 `knowledge.search` 与 `knowledge.show` 各自完整 primary union、`context={}`、无 supplemental、Human 中文与 `0/2/1` normal exit。因此八条 public command path 均已有 command-owned concrete binding；[Knowledge Ask Observable v1](./knowledge-ask-observable-v1.md) 已冻结 `knowledge.ask` 的五项 command-owned supplemental、Human 中文/exit、Retrieval Audit/View 测量与 presentation-failure 可观察边界；未被 T02 typed-verdict/grammar table 分类的 internal/entry fault 及新合同明确排除的其他路径仍不能冒充 handled diagnostic。八条 public command 的合同边界均已有权威来源，但这不宣称实现已 production complete。ADR 0108 另行冻结 controlled `knowledge.ask --json` presentation failure 为不生成 diagnostic 的静默 `os._exit(1)`；Operations v1 显式采用自己的 settled-resource presentation failure；[ADR 0116](../adr/0116-return-2-with-one-fixed-stderr-line-for-raw-argv-resource-violation.md) 冻结 raw argv resource violation 为 fixed stderr/empty stdout/exit `2`，[CLI Command v1](./cli-command-v1.md) 冻结不生成 `CliDiagnosticItemV1` 的 controlled entry failures。决策依据见 [ADR 0091](../adr/0091-use-two-field-code-discriminated-cli-diagnostics.md) 至 [ADR 0099](../adr/0099-prove-no-commit-safety-with-a-zero-live-resource-ledger.md)、[ADR 0108](../adr/0108-return-1-for-controlled-knowledge-ask-json-presentation-failure.md)、[ADR 0116](../adr/0116-return-2-with-one-fixed-stderr-line-for-raw-argv-resource-violation.md) 与 [ADR 0120](../adr/0120-keep-operations-read-only-and-report-partial-state-conservatively.md)，outer 见 [CLI JSON v1](./cli-json-v1.md)。

对 `knowledge.ask`，Diagnostic set 只有在 [ADR 0100](../adr/0100-seal-the-handled-cancellation-window-before-presentation.md) 的 final command-state seal 中与 `outcome`、`result` 一起锁存后才是可呈现终态，且 presentation 只能在所选 cancellation profile 完成 release proof 后开始。`doctor/status`、三个 Literature binding 与 `knowledge.search/show` 都不建立 Knowledge Ask cancellation profile；[Operations v1](./operations-v1.md) 在全部只读 handle/child 已 settle 后使用自己的 immutable report seal，[Literature Commands v1](./literature-commands-v1.md) 在全部 command-owned child/write/handle/ownership 已 settle/release 后使用自己的 immutable outcome/buffer seal，[Knowledge Read v1](./knowledge-read-v1.md) 则在只读 operation、result/diagnostic validation 与 resource close 完成后锁存 immutable command report。四类 binding 在各自 seal 后都不得增加、删除、重排或重分类 item。
...
[Knowledge Ask Observable v1](./knowledge-ask-observable-v1.md) 只把其五个 code 加入 `knowledge.ask` 的 supplemental union，并复用本合同的 role、ASCII 排序、16-item/16,384-byte cap 与 omission-last；supplemental count 无法表示时使用该 command-owned typed no-output presentation boundary，不改写 primary、outcome、result 或本共享 profile。
...
Human renderer 直接消费同一个已验证 diagnostic set，并按 code/context 映射中文说明和处理建议；machine JSON 不保存 message 或 remediation。JSON writer 不拥有 code 选择、领域聚合、Human 文案或隐私清洗。`knowledge.ask` 的 exact 原因、提示、下一步、`status` 与 `doctor` 职责分界由 [Knowledge Ask Observable v1](./knowledge-ask-observable-v1.md) 独占；共享层不得复制或同义改写。

三个 Literature binding 的完整映射见 [Literature Commands v1](./literature-commands-v1.md)：每个 blocked/failed receipt 恰好一个 primary，`succeeded` 为空数组，V1 不允许 supplemental；同 gate 多故障按 command-owned field 顺序 fail-fast，已经启动的 operation 形成多个候选时按静态 code/context priority 仲裁。Awaiting-review 的逐 Candidate Human 命令行、所有 primary 原因/下一步、单末尾 LF、32,768-byte cap 与 normal exit `0/2/1` 也在那里冻结，不再属于共享待决项。
