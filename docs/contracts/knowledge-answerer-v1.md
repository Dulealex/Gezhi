...
- `knowledge.ask --json` 的 supplemental code/context union、Human 中文文案/exit、未被 [CLI Command v1](./cli-command-v1.md) typed-verdict/grammar table 分类的 internal/entry fault、ADR 0108 排除的 JSON failure 与其他 Human presentation failure exit；controlled `CLI_BOOTSTRAP_FAILED`/`CLI_ARGUMENT_FAILED` receipt 已由 T02 冻结；Answer 逐路径与整目录读取额度、精确 identity、orphan recovery 及 V1 显式 maintenance 空动作集已由 [Answer Terminal v1](./answer-terminal-v1.md) 冻结；capture overflow 专属 supplemental diagnostic、`retrieval_view_too_large` 诊断的精确 audit 字段与其规范序列化，以及未来显式 maintenance interface；内部 Job stop DWORD 已由 [Codex Child Process v1](./codex-child-process-v1.md) 冻结为 `0x475A0001`。
