...
ADR 0111 与 [ADR 0119](../adr/0119-lazy-load-only-the-selected-context-command-adapter.md) 共同冻结强入口 import ordering：最小 project bootstrap 只在 `RawArgvPreflightV1` PASS 且 typed checks 通过后加载 Typer、Rich 与 inert full routing graph，并只在 valid daily command selection 后加载 selected owning adapter；解释器、site、entry stub 与最小 bootstrap/preflight module 自身仍位于该保证之前。
...
[ADR 0111](../adr/0111-lazy-import-the-cli-framework-and-command-graph-after-argv-preflight.md) 与 [ADR 0119](../adr/0119-lazy-load-only-the-selected-context-command-adapter.md) 冻结 preflight PASS 后先完成 typed checks、再加载 Typer/Rich 与 inert routing graph、并在 valid daily command selection 后加载 selected owning adapter 的强入口边界。
...
正常检索在最终选择形成后，必须先完整形成并验证 `RetrievalViewV1` value，随后恰好一次 canonical-serialize 并追加 LF，冻结同一 immutable measured View buffer、实际 byte length、SHA-256 与 cap verdict。只有该 measurement 已冻结，才形成并安装引用它的合法 `retrieval_audit.json`，持久阶段推进到 P3；`within_limit` 时再把同一个 measured buffer 安装为 `retrieval_view.json` 并推进到 P4，禁止重新序列化或替换为等值副本。`too_large` 时只安装 Audit 并固定停在 P3，不安装 View。该 ADR 0122 顺序取代任何“先逻辑形成 Audit、再形成 View”的旧表述，但不改变持久安装的 P3 Audit → P4 View 顺序：
...
`retrieval_audit.json` 使用 Python 3.11 `json.dumps(ensure_ascii=False, allow_nan=False, sort_keys=True, separators=(",", ":"))` 的 UTF-8 bytes 并追加一个 LF；无 BOM，物理换行只使用 LF。`retrieval_view.json` 则必须逐 byte 等于前段已经唯一形成的 measured View buffer，不得再次序列化。Manifest 分别记录实际 byte length 与 SHA-256；Audit measurement、View manifest pair、View 实际 bytes/hash 与 canonical View bytes必须完全相等。Measured View 的上限为 262144 bytes（256 KiB）inclusive；262144 合法，262145 固定得到 `blocked: retrieval_view_too_large`，不安装 View且不创建 Codex attempt。

超限时禁止截断短证据摘录、删除尾部 Candidate、降低 12 条选择结果、删除 Descriptor、只取摘要、调用第二轮模型压缩、切换模型或以 `retrieval_audit.json` 代替语义 View。有效的 Question、搜索投影、最终选择与超限 measurement 由合法的 `retrieval_audit.json` 保留，并按后文 P3 前缀发布为 blocked 审计目录；would-be View 的超限 bytes 只能留在内存或 writer-private 临时位置，terminal manifest 前必须释放或撤销，不得以正式 `retrieval_view.json` 进入 terminal Answer。[Knowledge Ask Observable v1](./knowledge-ask-observable-v1.md) 已冻结 `RetrievalAuditV1` 的精确字段、canonical bytes、same-snapshot identity、measurement 与 262144/262145 分支；实现不得自行发明或省略字段。以后需要更紧凑或更大的 View 时必须创建新角色或 Schema 版本。
...
在当前锁定 Codex CLI `0.146.0` profile 下，只有通过机械 watchdog、capture 与 lifecycle 完整性门禁明确分类的上述 30 分钟 wall-clock `timeout` 可以触发自动重试。该 profile 没有把 provider network、HTTP 429 或 HTTP 5xx 投影为格致可机械判别的字段；实现不得根据 `error.message`、`turn.failed.error.message`、stderr、final message、未知退出码文本或模型自然语言猜测这些分类。已安全收尾的其他 provider terminal、未知非零退出或事件结构失败一律保守归为不可重试的 `process_error`。第一次 timeout 后固定等待 10 秒，第二次 timeout 后固定等待 30 秒；最多重试两次，因此同一 Answer 最多创建 3 个 Knowledge Codex attempt。进程/provider 捕获正常结束并把可选 final text 交给 validation 时，synthesis launch 序列立即停止且关闭 attempt window，不创建多余 attempt；后续 validation 失败不得重新打开 window 或触发重试。
...
当重试序列因三次机会用尽，或 95-minute deadline 在 backoff/下一次 commitment 前结束 window 时，只使用已经创建并冻结的真实 timeout items，返回 `blocked: codex_timeout_exhausted`。Deadline 在活跃 attempt 内触发、且没有观察到 capture overflow 的 item 通过下文完整性门禁后最终分类为 `timeout` 时，这个真实值参与耗尽判断；对同样没有 overflow 的 item，若完整性门禁把它裁决为 `process_error`，则立即走 `failed: codex_process_failed`，不作为 timeout 耗尽或重试。已经锁存 overflow 的 item 固定为 `process_error`，不进入 timeout 集合；它在安全收尾并按 ADR 0081 冻结必填 usage 字段后以 `failed: codex_process_failed` 结束 Answer，禁止 backoff 与后续 attempt。Deadline 在 backoff 或下一次 commitment 前胜出时，不创建占位 item、不回写既有 item，也不向分类集合注入虚构的 timeout；若用户取消早于或等于这个固定 deadline，则改为锁存顶层 `interrupted`。因此因 window 提前结束而只有一项或两项真实 timeout attempt 时，仍使用 `codex_timeout_exhausted`。每个 attempt 的实际事件捕获 bytes 和最终 `failure_class` 都必须保留在 Answer staging；manifest 中的 attempt item 由下文封闭十字段记录拥有，原始捕获文件布局与 Answer 级 usage totals 已由后文/前文分别冻结，目录提交遵守已经冻结的 Answer 原子边界。

既有 manifest reader/validator 继续接受已经冻结的 `network`、`rate_limit`、`server_error` failure class，以及 `codex_network_exhausted`、`codex_rate_limit_exhausted`、`codex_server_error_exhausted`、`codex_transient_exhausted` diagnostic，以保持 V1 Schema 和历史记录兼容；当前锁定 profile 的新 writer 不得生成这些值。只有新的锁定 CLI/profile 通过新版本化决策暴露机器可判别字段后，才可恢复相应 writer 分类。
...
进入运行终态时，Python 先完成、验证并关闭 terminal manifest 以外的全部应保留资产并撤销全部私有临时 entry，再按 ADR 0087 直接排他创建、写入、验证并关闭字面 terminal `manifest.json`；只有所有文件与目录句柄都已关闭，才能把整个 staging 目录同卷原子改名为目标目录。目标必须不存在；如果目标已经存在或原子改名失败，禁止覆盖、合并、删除目标后重试、逐文件移动或以复制后删除模拟成功提交。此时 staging 仍不是正式 Answer，并按后文已冻结的保守恢复策略处理；确定的 target-exists 与其他确定 rename failure 分别使用 ADR 0095 的 `knowledge.ask.answer_target_conflict.v1` 与 `knowledge.ask.answer_commit_failed.v1`，V1 显式 maintenance action set 保持为空且未授权任何修复方式；rename 结果不确定时位于正常矩阵外。
...
Codex 原始事件和已经批准保留的失败现场继续受封闭资产清单约束；其他原始诊断只有在以后明确冻结独立、版本化、限长且经过敏感信息边界设计的审计资产后才能持久化，不能先塞入 manifest。未来确需新增 error 字段时必须升级 Answer manifest Schema，不能让 v1 reader 忽略未知字段。`code` 与 `stage` 的封闭枚举由紧随其后的章节冻结；CLI Human 文案映射已由 [Knowledge Ask Observable v1](./knowledge-ask-observable-v1.md) 冻结，其他持久诊断资产 Schema 未获授权且不得写入 Answer。
...
该门禁禁止解码器自行生成 replacement character，不禁止原始 bytes 合法编码的 U+FFFD，也不禁止 JSON string 内合法的 U+FEFF；这些 code point 仍由正常 JSON、Schema 与字符串规则判断。非 overflow `events.jsonl` 的 BOM/解码失败进入 lifecycle 的 event-stream 结构失败并得到 `process_error`，原始 capture 仍作为合法 asset 保留。只有最后一个 `failure_class=null` attempt 的 eligible final 才执行既有原始 byte-length 门禁和严格解码；其 BOM/解码失败得到 `answer_output_invalid`，不得把 attempt 改成 `process_error`。ADR 0078 的 overflow prefix 不是完整语义输入，允许结束在 UTF-8 sequence 中间；overflow 的 `process_error` 直接来自 latch 与 ADR 0080，不因 prefix 解码成功而清除，也不因解码失败改成普通 event-stream failure 或 eligible-final `answer_output_invalid`。Final prefix 永不作为 Answer validation 输入；[Knowledge Ask Observable v1](./knowledge-ask-observable-v1.md) 的外部 supplemental 只投影 overflow channel，不解码、摘录或暴露 prefix 内容。ADR 0081 令 usage consumer 对任何 exact-cap events 跳过严格解码，低于 cap 时才运行本门禁；这不禁止 non-overflow exact-cap clean EOF 为非 usage 终态分类目的进入权威 event adapter。Event framing 由下一节与 ADR 0075 冻结；精确 capture cap 见前节与 ADR 0077。
...
- [Knowledge Ask Observable v1](./knowledge-ask-observable-v1.md) 与 ADR 0122 已冻结 `knowledge.ask` 的五个 supplemental code/context、Human 中文/exit、Retrieval Audit/View measurement、`retrieval_view_too_large` 以及 JSON/Human presentation failure 的公开边界；它们不改变本合同的 pre-ID、Answer ID、primary/result、P0–P4、recovery或 JSON 正常路径。Answer 逐路径与整目录读取额度、精确 identity、orphan recovery 及 V1 maintenance 空动作集继续由 [Answer Terminal v1](./answer-terminal-v1.md) 拥有，内部 Job stop DWORD继续由 [Codex Child Process v1](./codex-child-process-v1.md) 冻结为 `0x475A0001`。未被 [CLI Command v1](./cli-command-v1.md) 或 Observable closed union 分类的 internal/entry、external termination、pending I/O 与 proof failure保持正常矩阵外；未来 maintenance 与持久 diagnostic 仍未获授权。
