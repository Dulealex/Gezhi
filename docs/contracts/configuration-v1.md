...
Context-only here is limited to business direct dependencies outside that bootstrap runtime import closure: Feedparser, HTTPX, Pydantic, Pydantic Settings, PyPDF, RapidFuzz, Tenacity, OCR, Codex, and future Context business dependencies. Under ADR 0119, the static routing descriptor and graph factory must not import or probe them; only a valid selected command may lazy-load its consuming handled adapter. Any subsequent ADR 0032 `open_gezhi()` composition freezes static ownership and deep-module interfaces, but does not authorize eager import or probing of unrelated Context-only business dependencies.
