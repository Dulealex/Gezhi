...
[ADR 0121](./0121-classify-continuation-failures-by-recoverability-and-certainty.md) 部分取代上一段对 `handoff_blocked` 与 `import_blocked` 的笼统失败映射：只有已知缺失且可恢复的前置条件使用 blocked；确定性的本地完整性、协议、revision、commit 或 Registry conflict 使用 failed，提交结果不确定则保持在正常 handled 矩阵外。
