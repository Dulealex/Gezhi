...
- **共同 CLI JSON seam**：所有公开命令通过 [CLI JSON v1](./docs/contracts/cli-json-v1.md) 的封闭五字段 outer 输出机器结果，并复用 [CLI Diagnostics v1](./docs/contracts/cli-diagnostics-v1.md) 的两字段 item、角色、排序、容量与隐私 profile；每个 concrete command 仍拥有领域 result、code/context union 与跨字段矩阵，[Knowledge Ask Diagnostics v1](./docs/contracts/knowledge-ask-diagnostics-v1.md) 是首个 committed-primary 与 no-commit-outcome/normal-exit binding，各上下文不复制共享 diagnostic-set module 或 JSON writer。
