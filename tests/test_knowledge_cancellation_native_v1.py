...
def test_native_seal_rechecks_poison_after_winning_the_gate(
    tmp_path: Path,
) -> None:
    dll_path = tmp_path / "gezhi_cancel_seal_poison_test_v1.dll"
    subprocess.run(
        [
            "pwsh",
            "-NoProfile",
    ...
    assert _run_native_probe_v1("seal-poison-race", dll_path) == {
        "mode": "seal-poison-race",
        "proof": "rejected",
    }


def test_native_seal_rechecks_poison_after_forming_the_transition(
    tmp_path: Path,
) -> None:
    dll_path = tmp_path / "gezhi_cancel_seal_commit_poison_test_v1.dll"
    subprocess.run(
        [
            "pwsh",
            "-NoProfile",
    ...
    assert _run_native_probe_v1("seal-poison-after-gate", dll_path) == {
        "mode": "seal-poison-after-gate",
        "proof": "rejected",
    }
